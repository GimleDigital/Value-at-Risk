...
package object algebra {
   ...
   */
  def newton(f: DenseMatrix[Double] => DenseMatrix[Double],
                j: DenseMatrix[Double] => DenseMatrix[Double],
                xk: DenseMatrix[Double]): DenseMatrix[Double] = {
    val x = xk - inv(j(xk)) * f(xk)
    val y = f(x) + j(x) * (x - xk)

    if (y.map(utils.rounded(_, 5)) == DenseMatrix.zeros[Double](y.rows, y.cols)) x
    else newton(f, j, x) // Not resolved, run next iteration
  }
